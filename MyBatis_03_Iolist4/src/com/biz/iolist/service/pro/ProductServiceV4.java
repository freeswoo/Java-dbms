...
public class ProductServiceV4 extends ProductServiceV3 {
	...
	/*
	 * 상품정보들을 입력받아서 insert 수행 상품코드를 입력받아서 있으면 다시 입력하도록 없으면 다음으로 진행 1. 상품코드를 어떻게
	 * 할것인가? 가. 자동으로 생성하기 나. 직접 입력하기(이미 문서로 작성된 경우)
	 * 
	 * 2. 상품이름을 입력하는데, 완전히 일치하는 상품이 이미 있는경우 가. 코드가 다르면 그냥 입력하기 나. 코드가 다르고 단가부분이 다르면
	 * 그냥 입력하기 상품정보 : 상품이름, 품목, 주매입처 다. 같은 상품이름 입력 금지
	 * 
	 * 3. 단가부분 가. 매입단가를 입력하면, 표준 판매단가를 계산하기 나. 매입단가, 매출단가를 별도로 입력 다. 매입단가일 경우
	 * vat포함여부 라. 매출단가, 소매점에서는 vat포함이 기본 도매점일경우 vat포함여부
	 */
